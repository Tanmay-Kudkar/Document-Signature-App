...
export default function SeveralPeopleModal({ onCancel, onApply }) {
  ...
  return (
    <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
      ...
      <div className="relative w-full bg-white rounded-2xl flex flex-col animate-fade-in" style={{ maxWidth: 850, maxHeight: '90vh', boxShadow: '0 24px 80px rgba(0,0,0,0.2)' }}>
        
        {/* ── MODAL HEADER ── */}
        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 shrink-0">
          <h2 className="font-bold text-gray-900 text-[24px]">Create your signature request</h2>
        </div>

        {error && (
          <div className="px-8 py-3 bg-red-50 border-b border-red-100 text-red-600 text-sm font-semibold flex items-center gap-2 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 custom-scrollbar">
          ...
          <div className="mb-10">
            <h3 className="text-[15px] text-gray-600 mb-4">Who will receive your document?</h3>
              ...
              {receivers.map((r, i) => (
                <div key={r.id} className="border-b border-gray-200 last:border-b-0">
                  <div className="flex items-center gap-3 p-3 bg-white">
                    ...
                    {/* Color dot */}
                    <div className="w-8 h-8 rounded-full shrink-0" style={{ background: RECEIVER_COLORS[i % RECEIVER_COLORS.length] }}></div>
                    ...
                    <input 
                      ...
                      className="w-1/4 border border-gray-300 rounded px-3 py-1.5 text-sm outline-none focus:border-blue-500 transition"
                    />

                    {/* Email */}
                    <div className="w-1/4 relative flex items-center border border-gray-300 rounded overflow-hidden focus-within:border-blue-500 transition">
                      <input 
                        type="email" 
                        placeholder="Email"
                        value={r.email} 
                    ...
                    <div className="flex items-center gap-1.5 ml-auto">
                      ...
                      <button onClick={() => removeReceiver(r.id)} className="w-8 h-8 flex items-center justify-center ml-2 text-gray-500 hover:text-red-500 transition">
                        <X size={18} />
                      </button>
                    </div>
                  </div>
                  ...
                  {(r.showPassword || r.showPhone || r.showFormat) && (
                    <div className="px-14 pb-3 flex flex-wrap gap-4 items-center bg-[#fafbfc]">
        ...
        {/* Footer */}
        <div className="shrink-0 flex items-center justify-end gap-3 px-8 py-4 border-t border-gray-100 bg-white rounded-b-2xl shadow-[0_-2px_10px_rgba(0,0,0,0.02)]">
          <button onClick={onCancel} className="font-bold text-[#e8222c] hover:underline px-4 transition text-[15px]">
            Cancel
          </button>
          <button onClick={handleApply} className="text-white font-bold px-8 py-2.5 rounded transition hover:opacity-90 text-[15px] shadow-sm" style={{ background: '#f58c8a' }}>
            Apply
          </button>
        </div>
